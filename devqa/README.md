...
- developer: 소문자, 라틴 표기 (`harry`, `vin`, `kelvin`, `john`, `bin`)
- 여러 개면 `-2`, `-3`… 뒤에 붙임

## 질문에 이미지가 딸려 오면 답변 페이지에도 반드시 넣는다
⚠️ 2026-08-17에 한 번 빠뜨려서 나중에 「어느 화면의 어느 문구」인지 알 수 없게 된 적이 있음(cms-bin-20260817.html). 원문에 `[image]`가 있으면 답변에도 반드시 넣을 것 — 질문 바로 아래(답변보다 위), 폭에 맞춰 작게 넣고 누르면 크게 보이게.
...
## 표기
- 개발자 이름은 라틴 표기 그대로 사용 (Harry / Vin / Kelvin / John / Bin) — 한국어 문장 안에서도 번역하지 않음.
- 허브(`devqa/index.html`)에 새 개발자가 추가되면 `.devtabs` 필터 버튼과 JS `DEV_TABS` 배열에도 추가해야 필터가 동작함.
